...
Um jogo RPG idle desenvolvido em console como projeto de aprendizado de **Programação Orientada a Objetos (POO)** em Python.
...
- **Linguagem:** Python 3
- **Bibliotecas:** `rich` (formatação visual), `random` (geração aleatória), `copy` (cópia de objetos), `json` (persistência de dados), `time` (controle de fluxo), `os`(limpeza do console),.
- **Ambiente:** PyCharm
...
-> ⚠️ **PyCharm:** Habilite *Emulate terminal in output* em `Run > Edit configurations > Edit configuration templates > Python > Modify options` para visualizar a interface colorida.
